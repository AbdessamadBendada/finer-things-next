/* Extracted from legacy/bespoke-accessories.html. Reference only — port into hooks. */
  ...
  if(!matchMedia('(prefers-reduced-motion:reduce)').matches){
    ...
    const updateProject=()=>{
      ...
      const progress=Math.max(0,Math.min(1,(innerHeight-rect.top)/(innerHeight+rect.height)));
      projectBg.style.transform=`translate3d(${(progress-.5)*12}px,${(progress-.5)*28}px,0) scale(1.025)`;
      queued=false;
    };
