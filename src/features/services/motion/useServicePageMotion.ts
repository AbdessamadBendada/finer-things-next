...
export function useServicePageMotion(
  ...
  // The selected-project title reveals at its own threshold.
  useEffect(() => {
    const element = root.current;
    if (!element) return;
    const disposers = [observeOnce(element, '.title-mask', { threshold: 0.45 })];
  ...
  const drive = useCallback(
    (element: HTMLElement) => {
      ...
      if (projectDrift && projectBg?.parentElement) {
